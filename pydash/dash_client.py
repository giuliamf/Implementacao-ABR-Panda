...
class DashClient:
    ...
    def __init__(self):
        ...
        r2a_algorithm = config_parser.get_parameter('r2a_algorithm')
        ...
        r2a_class_name = ''.join(word for word in r2a_algorithm.split('_'))

        r2a_class = getattr(importlib.import_module('r2a.' + r2a_algorithm.lower()), r2a_class_name)
    ...
    def handle_scheduler_event(self, event):
        print(f"Evento agendado: {event.get_msg()} para o módulo {event.get_dst()}")
        # checking if the event is inside the modules position range limits
        if event.get_dst() < 0 or event.get_dst() >= len(self.modules):
            print(
                f'It is no possible to route a {event.get_msg()} message from {event.get_src()} to {event.get_dst()} '
                f'module list position.')
            exit(0)
